...
const ShopScreenCategoryPresenter = ({navigation, parentProps, ...props }) => {
	...
	React.useEffect(()=>{
		...
		updateSelected(parentProps.items.subCategories[0]['subCategoryId']);
		return () => {
	...
	const updateSelected = (subCategoryId) => {
		...
		if(parentProps.subCategoryId!==null && selected===null){
			...
			//this is when user has not selected a product before hand and only is exposed to category
	...
	if(selected===null){
		...
		content = (
			<Theme>
				...
				/>
				<SubCategoryList 
					subCategories={parentProps.items.subCategories}
					selected={selected}
